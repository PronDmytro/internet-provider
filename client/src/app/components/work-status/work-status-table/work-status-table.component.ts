...
import {WorkStatusFormDialogComponent} from '../work-status-form-dialog/work-status-form-dialog.component';

@Component({
  selector: 'app-work-status-table',
  templateUrl: './work-status-table.component.html',
  styleUrls: ['./work-status-table.component.scss'],
})
export class WorkStatusTableComponent implements OnInit {

  @Input() public data: WorkStatus[] | undefined;

  public displayedColumns: string[] = ['name', 'btn'];
  // @ts-ignore
  public dataSource: MatTableDataSource<WorkStatus>;
  // @ts-ignore
  ...
  private initMatTable() {
    if (this.data) {
      this.dataSource = new MatTableDataSource<WorkStatus>(this.data);
  ...
  public createData(): void {
    ...
    const dialogRef = this.dialog.open(WorkStatusFormDialogComponent,
      dialogConfig);
  ...
  public editData(data: WorkStatus) {
    const dialogConfig = new MatDialogConfig();
  ...
  public deleteData(data: WorkStatus) {

    this.apiRequestsService.deleteWorkStatusData(data).subscribe({
      next: (response) => {
